...
if(! defined('MEMORY_LIMIT') )				        define ('MEMORY_LIMIT', '512M');							// Set PHP memory limit
if(! defined('BLOCK_EXTERNAL_LEECHERS') ) 	        define ('BLOCK_EXTERNAL_LEECHERS', false);				// If the image or webshot is being loaded on an external site, display a red "No Hotlinking" gif.

//Image fetching and caching
if(! defined('ALLOW_EXTERNAL') )			        define ('ALLOW_EXTERNAL', config('image.allow_external_sites'));						// Allow image fetching from external websites. Will check against ALLOWED_SITES if ALLOW_ALL_EXTERNAL_SITES is false
if(! defined('ALLOW_ALL_EXTERNAL_SITES') ) 	    define ('ALLOW_ALL_EXTERNAL_SITES', config('image.allow_all_external_sites'));				// Less secure.
...
if(! defined('FILE_CACHE_DIRECTORY') ) 		    define ('FILE_CACHE_DIRECTORY', Storage::disk(config('image.cache.filesystem'))->path(config('image.cache.folder')));				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
    ...
    protected function processImageAndWriteToCache($localImage){
        ...
        if (function_exists ('imagefilter') && defined ('IMG_FILTER_NEGATE')) {
            $imageFilters = array (
                ...
                4 => array (IMG_FILTER_CONTRAST, 10),
