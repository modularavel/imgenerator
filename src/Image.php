...
if (! defined('ALLOW_EXTERNAL')) {
    define('ALLOW_EXTERNAL', config('image.allow_external_sites'));
...
if (! defined('ALLOW_ALL_EXTERNAL_SITES')) {
    define('ALLOW_ALL_EXTERNAL_SITES', config('image.allow_all_external_sites'));
}				// Less secure.
...
if (! defined('FILE_CACHE_PREFIX')) {
    define('FILE_CACHE_PREFIX', 'cache');
}				// What to put at the bag of all files in the cache directory so we can identify them

if (! defined('FILE_CACHE_DIRECTORY')) {
    define('FILE_CACHE_DIRECTORY', Storage::disk(config('image.cache.filesystem'))->path(config('image.cache.folder')));
...
if (! isset($ALLOWED_SITES)) {
    $ALLOWED_SITES = config('image.allowed_external_hosts');
}
...
class Image
{
    protected string|array|null $src = '';
    ...
    #[NoReturn] public static function start()
    {
        $tim = new Image();
        $tim->handleErrors();
        $tim->securityChecks();
    ...
    protected function setMemoryLimit()
    {
        ...
        $inibytes = Image::returnBytes($inimem);
        $ourbytes = Image::returnBytes(MEMORY_LIMIT);
    ...
    protected function getURL($url, $tempfile)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            curl_setopt($curl, CURLOPT_WRITEFUNCTION, 'Image::curlWrite');
