...
use Modularavel\Imgenerator\Exceptions\NoImageSpecifiedException;
use Throwable;
...
if (!defined('ALLOW_EXTERNAL')) {
    define('ALLOW_EXTERNAL', config('imgenerator.allow_external_sites'));
...
if (!defined('ALLOW_ALL_EXTERNAL_SITES')) {
    define('ALLOW_ALL_EXTERNAL_SITES', config('imgenerator.allow_all_external_sites'));
}                // Less secure.
...
if (!defined('FILE_CACHE_TIME_BETWEEN_CLEANS')) {
    define('FILE_CACHE_TIME_BETWEEN_CLEANS', 2592000000);
}    // How often the cache is cleaned

if (!defined('FILE_CACHE_MAX_FILE_AGE')) {
    define('FILE_CACHE_MAX_FILE_AGE', 2592000000);
}                // How old does a file have to be deleted from the cache
...
if (!defined('FILE_CACHE_PREFIX')) {
    define('FILE_CACHE_PREFIX', config('imgenerator.cache.prefix'));
}                // What to put at the bag of all files in the cache directory so we can identify them

if (!defined('FILE_CACHE_DIRECTORY')) {
    define('FILE_CACHE_DIRECTORY', Storage::disk(config('imgenerator.cache.filesystem'))->path(config('imgenerator.cache.folder')));
}                // Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)

if (!defined('MAX_FILE_SIZE')) {
    define('MAX_FILE_SIZE', 20485760);
...
if (!defined('BROWSER_CACHE_MAX_AGE')) {
    define('BROWSER_CACHE_MAX_AGE', 864000000);
...
//Image compression is enabled if either of these points to valid paths
...
if (!isset($ALLOWED_SITES)) {
    define('ALLOWED_SITES', config('imgenerator.allowed_external_hosts'));
}
...
class Imgenerator
{
    ...
    protected $cropTop = false; //Generally if timimage.php is modifed (upgraded) then the salt changes and all cache files are recreated. This is a backup mechanism to force regen.
    protected $salt = ''; //Designed to have three letter mime type, space, question mark and greater than symbol appended. 6 bytes total.
    ...
    public function __construct()
    {
        ...
        if (FILE_CACHE_DIRECTORY) {
            ...
            if (!touch($this->cacheDirectory . '/index.php')) {
                $this->error('Could not create the index.php file - to fix this create an empty file named index.php file in the cache directory.');
            }
        } else {
        ...
        throw_if((Str::length($this->src) <= 3), NoImageSpecifiedException::class);
    ...
    protected function getURL($url, $tempfile)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            curl_setopt($curl, CURLOPT_WRITEFUNCTION, 'Imgenerator::curlWrite');
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        if (($origType == IMAGETYPE_PNG || $origType == IMAGETYPE_GIF) && function_exists('imageistruecolor') && !imageistruecolor($image) && imagecolortransparent($image) > 0) {
    ...
    protected function setMemoryLimit()
    {
        ...
        $inibytes = Imgenerator::returnBytes($inimem);
        $ourbytes = Imgenerator::returnBytes(MEMORY_LIMIT);
